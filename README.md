...
| **하이브리드 검색** | ChromaDB + sentence-transformers (all-MiniLM-L6-v2) + BM25 (rank_bm25) |
...
음악치료에서 사용하는 동질 원리를 적용합니다. 사용자의 현재 감정(valence + energy)에서 목표 감정까지 2D 유클리드 거리 기반으로 점진적으로 변화하는 플레이리스트를 구성하여 자연스러운 감정 이동을 유도합니다.
...
1. **풍부한 규칙 기반 문장화**: 11개 오디오 피처(energy, valence, danceability, tempo, acousticness, instrumentalness, speechiness, loudness, liveness, mode, key)를 5단계 한국어 키워드 + 수치값으로 변환하여 임베딩 텍스트 생성
2. **배치 임베딩**: sentence-transformers로 512개씩 배치 인코딩 (개별 대비 ~10배 빠름)
3. **ChromaDB 저장**: 영구 저장소(`data/chroma_db/`)에 임베딩 + 메타데이터(11개 오디오 피처 전체) 저장
4. **증분 처리**: 이미 임베딩된 곡은 건너뜀

임베딩 텍스트 예시:
```
아티스트의 '곡명'은(는) energy 0.72, valence 0.65, danceability 0.81, ... 활기찬, 밝고 긍정적인, 춤추기 좋은, 빠른 템포의, 전자음 위주의, 보컬 중심의, ... 장조(밝은) A키 pop 곡입니다.
```
...
**하이브리드 검색** (`app/tools/search_by_description.py`):
- **벡터 검색**: ChromaDB 쿼리 (자연어 → sentence-transformers 임베딩 → 의미 유사도)
- **BM25 키워드 검색**: 임베딩 텍스트에 대한 정확 키워드 매칭 (rank_bm25)
- **점수 합산**: `final = 0.6 × vector_score + 0.4 × bm25_score`로 융합
- 벡터 검색은 의미적 유사성, BM25는 키워드 정확 매칭을 보완하여 검색 품질 향상
...
- **케어 노트**: 반복 패턴 메모 (최대 20개, 예: "주로 저녁에 우울 관련 추천 요청")
...
| **가드레일 통과율** | 금지 표현 미포함 비율 | 100% |
| **감정 추정율** | emotion intent 쿼리에서 감정 추정 성공 비율 | >= 80% |
...
│   │   ├── search_by_description.py # 하이브리드 검색 (벡터 + BM25)
...
├── requirements.txt
├── .env.example
├── reset.py                      # 데이터 초기화 스크립트
├── qa_report.md                  # 기획안 vs 구현 대조 QA 리포트
└── troubleshooting.md            # 개발 과정 이슈/해결 기록
```
